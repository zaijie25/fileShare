...
export default class GameListView extends ViewBase {
    ...
    {
        let top = cc.v2(this.gameListScroll.getScrollOffset().x,0)
        let bot = cc.v2(this.gameListScroll.getScrollOffset().x,this.gameListScroll.getMaxScrollOffset().y)
        let offSet = this.gameListScroll.getScrollOffset()

        let topXOffset = offSet.x - top.x
        let topYOffset = offSet.y - top.y
        let topDistence =  Math.sqrt(Math.pow(topXOffset, 2) + Math.pow(topYOffset, 2))

        let botXOffset = offSet.x - bot.x
        let botYOffset = offSet.y - bot.y
        let botDistence =  Math.sqrt(Math.pow(botXOffset, 2) + Math.pow(botYOffset, 2))
    ...
    public async refreshGameList(enterGameOffsetX:number, callback = null) {
        this.hideAllItem();
        ...
        let gameList = Global.GameData.hallGameList;//是否是更多游戏
        let spaceX = Global.Setting.SkinConfig.hallGameListOffsetX; //间隔
        let gongGaoNodeWide = this.gonggaoNode.width;   //banner的宽
        let moreGameNodeWidth = 0;   //更多游戏按钮宽
        this.gameItemList = [];
        let rightNodeArr = [];
        let count = 0;
        let startingDistance = gongGaoNodeWide;
        for (let i = 0; i < gameList.length; i++) {
            let gameData = gameList[i];
            //过滤部分游戏
            if (AppHelper.isBaiduSpecialState() && AppHelper.isFilterGame(gameData.game_id))
                continue;
