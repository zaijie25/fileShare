...
class ItalkCrc {
    ...
    private static crc16Tab:Array<number> = new Array(
    ...
    static crc16Add(crc:number,c:number):number{
        return this.crc16Tab[((crc >> 8) ^ c) & 0xFF] ^ ((crc << 8) & 0xFFFF);
    }

    static crc16(str:Uint8Array):number{
        let n:number;
        let crc:number = 0;
      for (n = 0; n < 6; n++) {
